...
function Lifecycle2() {
    ...
    const old = useRef(null); // Valeur conservées entre les rendus

    useEffect(() => {
        console.log('avant 2', old);
        console.log('apres', degrees);

        if (degrees < 0 && old.current === 0) {
            console.log('Trop froid');
        } else if (degrees > 0 && old.current === 0) {
